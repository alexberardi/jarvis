...
**Re-arm parked features (non-clarify parks).** A feature on `status:blocked` whose `feature-state.blocked_on` is `"coverage-gap: …"`, `"stale-case: …"`, or `"case-catalog-missing"` (NOT `"clarify"`) is eligible to UN-PARK when its blocker clears — check this here so these parks don't strand forever:
- If a `<!-- retry-please:v1 -->` comment exists with `created_at` AFTER the park's feature-state comment, OR the blocking CASE now resolves — for `coverage-gap`/`stale-case`: qa-author has posted a `🔔 qa-author:` comment AND the named CASE now resolves in the catalog (Step 7 catalog-resolution — i.e. its test PR merged); for `case-catalog-missing`: the CASE catalog / the named CASE now resolves → clear `status:blocked` and drop **`needs:alex` AND `needs:qa-author`** (per §Label & close operations: read `get_labels` → drop `status:blocked`, `needs:alex`, and `needs:qa-author` → `issue_write` update `labels=<remaining full set>`), set `blocked_on: null` in a fresh `feature-state:v1` comment, re-derive `human_locked` from the current presence of `status:locked`, and re-run Step 7 from the top for this feature.
...
**Post-CI fail-closed → park + route to qa-author (the #51/CASE-225 pattern).** For a feature that already has a `<!-- coding-agent-feature-ready:v1 -->` sentinel (coding-agent opened its PR set) and whose latest `<!-- qa-execution-report:v1 -->` is a **fail-closed "cannot verify" / RED caused by an EXISTING CASE asserting pre-fix behavior that this feature deliberately changes** (a STALE test, NOT a genuine regression): PARK it — read `get_labels` → **add `status:blocked` + `needs:qa-author`, REMOVE `status:ready-for-code`** → `issue_write` update `labels=<resulting full set>`; set `blocked_on: "stale-case: CASE-NNN"` in a fresh `feature-state:v1`; and post a `🔧 engineering:` comment naming the exact CASE id + the corrected assertion qa-author must apply (e.g. "CASE-225: assert 422, not 200"). **Do NOT leave it `status:ready-for-code`** — coding-agent re-scans that label every run (the benign no-op loop) — and **do NOT dead-end on `needs:alex`**: `needs:qa-author` routes the fix straight to qa-author, which updates the test and reports back; the re-arm logic above then un-parks it once the CASE resolves. **Judgment call:** if the new code is actually WRONG (a real regression, the test was right), that is a coding-agent fix via a `<!-- retry-please:v1 -->` re-arm, NOT a qa-author test edit — read the qa-execution-report detail before deciding which.
...
**Coverage-gap policy — BLOCK (condition 0).** If the QA test plan's fenced block carries a non-empty `proposed_cases: [...]` (QA judged the generic CASE-401/402 probes do NOT exercise the feature's actual behavior and a feature-specific CASE is missing): do NOT set `status:ready-for-code`. Instead set `status:blocked` + **`needs:qa-author`** (per §Label & close operations: read `get_labels` → add `status:blocked` and `needs:qa-author` to the full set → `issue_write` update `labels=<full set>`), set `blocked_on: "coverage-gap: <proposed case>"` in the feature-state object, **post a brief `🔧 engineering:` comment naming exactly which CASE id(s) qa-author must author and the behavior each must assert** (the plan's `proposed_cases` ARE the spec), and PARK. Setting `needs:qa-author` routes the missing-CASE work straight to qa-author — its Step 0 interrupt path picks it up via the webhook and authors the CASE. **Do NOT dead-end on `needs:alex`** for a missing-CASE park (that was the stall bug); qa-author escalates to Alex itself only if it genuinely cannot fulfill the ask. A `proposed_cases`-bearing plan is ALWAYS a park, never a ready plan, regardless of `unit_cases`/`integration_cases` content. The feature stays parked until qa-author's CASE PR merges and the case resolves in the catalog (or Alex re-arms via a `<!-- retry-please:v1 -->` comment) — coding-agent is FORBIDDEN from touching the integration-tests harness; qa-author is the sole CASE author. (Stronger "green means it works" over loop velocity — Alex's decided call.)
