...
1. Call `mcp__github-rw__list_issues` on `alexberardi/jarvis-roadmap` with `state=open`. Query the labels where a QA plan can be owed (so an amended-after-ready feature still gets a refreshed plan — see step 3's refresh predicate): **`status:locked` OR `status:ready-for-code` OR `status:in-progress`** (do this as up to three list calls or one filtered scan; exclude terminal labels). Sort oldest first. (`status:locked` = Alex's explicit "go" — only locked-or-later features are real work. Engineering and Alex own all labels; you only read them. QA must run while a feature is merely `status:locked` because engineering only sets `status:ready-for-code` AFTER your plan exists; the later labels are for the amendment-refresh path.)
...
```yaml
feature_key: "<feature_key — cross-repo-lane repos only>"
unit_cases:
...
- `mcp__github-rw__list_issues` — find candidate umbrellas (`status:locked` / `status:ready-for-code` / `status:in-progress`, oldest first).
- `mcp__github-rw__issue_read` (method `get_comments`) — read the sentinel comment threads. (SHARED-SPEC §13 unifies all three personas on this exact method name; if the live github-rw MCP server instead exposes a distinct `list_issue_comments` tool, all three contracts + the spec line are updated together — do not let this contract diverge.)
